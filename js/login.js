...
const linkAdministrador = document.querySelector("#linkAdmin");

const expRegCorreo =
  /^[a-z0-9!#$%&'+/=?^_`{|}~-]+(?:.[a-z0-9!#$%&'+/=?^_`{|}~-]+)@(?:[a-z0-9](?:[a-z0-9-][a-z0-9])?.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/;
const expRegContrasenia =
  /^(?=.*\d)(?=.*[\u0021-\u002b\u003c-\u0040])(?=.*[A-Z])(?=.*[a-z])\S{8,16}$/;
...
function verificarUsuario() {
  ...
  if (
    email.value.match(expRegCorreo) === null ||
    password.value.match(expRegContrasenia) === null
  ) {
